...
export default function ToonGallery() {
  const { t } = useI18n();
  const {
    items,
    platform,
      ...
      <div className="mt-8 flex flex-wrap gap-2">
        {platforms.map((p) => (
          <button
            ...
            className={`border-2 px-4 py-1.5 font-mono text-xs transition-colors ${
              p === platform
                ? "border-accent bg-accent font-black text-background"
                : "border-foreground/20 bg-card text-muted hover:border-accent hover:text-accent"
            }`}
          >
      ...
      <div className="mt-8 flex flex-wrap items-center gap-4">
        {total > 1 && (
          <button
            ...
            className="inline-flex items-center gap-2 font-mono text-sm font-bold text-accent underline underline-offset-4 transition-colors hover:text-accent-2"
          >
