    ...
    <section id="blog" className="mx-auto max-w-5xl px-6 py-16 md:py-24">
      ...
      <div className="mt-12 space-y-5">
        {posts.map((post, i) => (
          <article
            key={post.id}
            className="card-in overflow-hidden rounded-3xl border border-border bg-card transition-all hover:-translate-y-1 hover:border-accent/50"
          ...
          >
            ...
            {open === post.id && (
              <div className="border-t border-border px-7 py-6">
                ...
                <Link
                  href={`/blog/${post.id}`}
                  className="pop-on-click mt-4 inline-block rounded-2xl bg-gradient-to-r from-accent to-accent-2 px-5 py-2.5 text-sm font-semibold text-background"
                >
      ...
      <div className="mt-10 text-center">
        <Link
          href="/blog"
          className="pop-on-click inline-block rounded-2xl border-2 border-border px-7 py-3 font-semibold transition-all hover:border-accent hover:text-accent"
        >
