...
export default function RetroGithub() {
  const { t } = useI18n();
  ...
  return (
    <Section id="github">
      ...
      <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((s) => (
          <Card key={s.key}>
            ...
            <p className="retro-glow mt-3 text-3xl font-black tabular-nums text-accent">
              {s.value}
            </p>
      ...
      <a
        ...
        className="mt-8 inline-flex items-center gap-2 rounded-md border border-accent/40 bg-card/80 px-5 py-2.5 font-mono text-sm transition-colors hover:border-accent hover:text-accent"
      >
