...
export default function NusantaraProjects() {
  const { t } = useI18n();
    ...
    <Section id="projects">
      ...
      <div className="mt-12 space-y-4">
        {t.default.projects.items.map((project, i) => (
          <article
            key={project.title}
            className="nusantara-panel group rounded-2xl p-6 transition-colors duration-500 sm:p-8"
          >
            <span
              aria-hidden
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                ...
                {project.demo && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 rounded-lg border border-border px-4 py-2 font-mono text-xs font-bold text-muted transition-colors hover:border-accent hover:text-accent"
                  >
                    demo ↗
                  </a>
                )}
