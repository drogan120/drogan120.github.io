...
export default function Ff7Skills() {
  const { t } = useI18n();
          ...
          <Card key={group.title} tag={group.icon}>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {group.skills.map((skill) => (
                <span
                  key={skill}
                  className="border border-accent/30 bg-background/60 px-3 py-1 font-mono text-[11px] text-muted transition-colors group-hover:border-accent/60 group-hover:text-accent sm:text-xs"
                >
                  {skill}
                </span>
              ))}
