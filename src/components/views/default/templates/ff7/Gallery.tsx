...
export default function Ff7Gallery() {
  const { t } = useI18n();
  const {
    items,
    platform,
        ...
        {platforms.map((p) => (
          <button
            ...
            className={`border px-4 py-1.5 font-mono text-xs transition-colors ${
              p === platform
                ? "ff7-glow border-accent bg-accent font-bold text-background"
                : "border-accent/30 bg-card/80 text-muted hover:border-accent hover:text-accent"
            }`}
          >
